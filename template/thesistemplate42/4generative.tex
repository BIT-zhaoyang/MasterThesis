...
\begin{align}
	\gamma(\mathbf{z}_n) = &p(\mathbf{z}_n \mid \mathbf{X}, \boldsymbol{\theta}^{old})	\\
	\gamma(\mathbf{z}_{n-1}, \mathbf{z}_n) = &p(\mathbf{z}_{n-1}, \mathbf{z}_n \mid \mathbf{X}, \boldsymbol{\theta}^{old}) \\
	Q(\boldsymbol{\theta}, \boldsymbol{\theta}^{old}) = &\sum_{k=1}^{K} \gamma(z_{1k})\ln \pi_k + \sum_{n=2}^{N}\sum_{j=1}^{K}\sum_{k=1}^{K}\gamma(\mathbf{z}_{n-1}, \mathbf{z}_n)\ln A_{jk}		\notag		\\
		&+ \sum_{n=1}^{N}\sum_{k=1}^{K}\gamma(z_{nk})\ln p(x_n\mid\phi_k)	
\end{align}
...
Computation in E-step is more difficult which requires efficient algorithm. The most widely used algorithm is known as \textit{alpha-beta} algorithm. The alpha-beta algorithm can be seen as an application of dynamic programming technique which takes advantage of the tree structure in HMM thus leading to efficiency. To start with the alpha-beta algorithm, following conditional independence properties should be obtained first
\begin{align}
	p(\mathbf{X}\mid \mathbf{z}_n) = &p(x_1, \cdots, x_n \mid \mathbf{z}_n)	\notag \\	
										&p(x_{n+1}, \cdots, x_N \mid \mathbf{z}_n) \\
	p(x_1, \cdots, x_{n-1}\mid x_n, \mathbf{z}_n) = &p(x_1, \cdots, x_{n-1} \mid \mathbf{z}_n)	\\
	p(x_1, \cdots, x_{n-1}\mid z_{n-1}, \mathbf{z}_n) = &p(x_1, \cdots, x_{n-1} \mid \mathbf{z}_{n-1})
\end{align}
These equations can be obtained by using \textit{d-seperation} technique~\cite{pearl2014probabilistic}, or proved formally using sum and product rules of probability~\cite{jordan2003introduction}.
